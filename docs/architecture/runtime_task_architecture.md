...
```text
与 ReviewDataset 导出等离线任务隔离；
不得因批量复盘导出或通知积压而无法及时运行；
...
交易关键任务优先于通知和离线复盘；
通知故障不阻断业务事实落库；
ReviewDataset 批量导出不影响编排、订单状态或成交同步；
...
不得在数据库事务中等待 Binance、Hermes、外部大模型或两秒轮询定时器。
...
订单提交后的状态与成交同步由独立订单生命周期同步任务组处理，不作为主交易编排尾部继续执行。
...
只有以下提交结果进入独立订单生命周期同步管线，并由该管线执行 OrderStatusSync：
...
→ 登记下一轮定向查询
→ 订单生命周期同步 run 保持 waiting

三十秒仍没有明确终态
→ 停止短轮询
→ 订单生命周期同步 run 结束为 unknown
→ ActiveLock 保持保护
→ 写必要 AlertEvent
```
...
同一 ReviewDataset 导出请求不创建重复有效数据集；
...
已通过权限检查的本轮不重新读取开关；
已经存在的 ExecutionPreparation、Execution 按本轮已冻结订单链继续；
已经登记的 OrderStatusSync 和 FillSync 按独立订单生命周期同步管线继续；
...
ReviewDataset 只能由 OpsConsole 导出入口或明确管理入口触发。
...
后台重复点击必须通过导出范围和输入指纹幂等返回已有有效结果。

### 18.2 ops_display 账户刷新

OpsConsole 账户刷新是独立后台任务或同步 service 调用，只生成 `ops_display` BinanceSyncRun。

它不加入自动 OrchestrationRun，不进入 OrderPlan、RiskCheck、ExecutionPreparation 或 ReviewDataset 的交易账户边界。
...
同一 ReviewDataset 导出范围不产生重复有效记录。
```
...
日志不得包含密钥、完整 Binance 响应、完整离线复盘输入输出或大体积业务数据。
...
Execution 通过交易关键队列完成主交易 run 的订单提交步骤；
OrderStatusSync 和 FillSync 通过独立订单生命周期同步 run 使用交易关键队列推进；
...
ReviewDataset 不阻塞交易关键任务；
Redis 与 result backend 丢失不破坏 MySQL 业务状态；
所有调度按 UTC。
```

测试默认使用 fake Gateway，不访问真实 Binance 或 Hermes，不提交真实订单。
...
交易关键任务与通知、ReviewDataset 逻辑隔离；
任务重投和崩溃恢复不会造成重复订单提交；
订单提交在任何层级都绝不重试；
关闭新交易权限不会中断既有订单追踪和成交同步；
RuntimeGuard 只读巡检且独立调度；
ReviewDataset 只由后台导出入口或明确受控管理入口触发；
所有计划时间和运行判断使用 UTC；
