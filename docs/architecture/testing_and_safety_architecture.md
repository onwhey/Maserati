...
实现一个模块时必须编写哪一层测试；
不同测试层分别验证什么；
测试如何替换 Binance、DeepSeek 和 Hermes；
如何验证重复任务不会重复下单；
如何验证异常状态会安全停止；
...
订单 accepted / unknown 进入 OrderStatusSync；
明确终态进入 FillSync；
...
AIReview 和 PerformanceMetrics 不占用交易关键任务资源；
...
端到端测试验证模块交接和整体结果，但不能替代每个模块的精细单元与合同测试。
...
不持有生产交易密钥；
不发送正式 Hermes 通知；
DeepSeek 调用默认使用 fake，确需真实调用时必须独立授权和限额。
```
...
在测试环境中如果代码尝试创建真实 Binance、DeepSeek 或 Hermes client，应立即失败，而不是静默访问网络。
...
直接发送 Hermes；
调用大模型；
巡检 PerformanceMetrics 或 AIReview。
```
...
```text
只使用相邻自动边界 trade_preparation 账户快照；
忽略 ops_display 和人工刷新；
NO_TARGET_CHANGE / NO_TRADE、no_strategy 或策略链路提前结束的周期仍可计算；
无下单周期仍可计算；
调仓影响按需求口径扣除；
缺少边界事实时不伪造收益；
重复一键补算只补缺失周期；
...
只读取已落库事实；
数据包完成脱敏和大小限制；
只通过 DeepSeekGateway 调用；
发送结果 unknown 时不自动重试；
非法模型输出不写成有效报告；
AIReviewFinding 和 Suggestion 只用于人工查看；
...
测试默认替换 Binance、DeepSeek 和 Hermes；
测试环境无法意外进入真实交易；
所有关键状态和故障路径有测试方法；
