...
自动四小时编排在起始阶段必须执行一次 `trade_preparation` 账户同步，以保存复盘周期边界事实，并作为后续可能进入订单链路时的账户事实。`NO_TARGET_CHANGE`、`NO_TRADE` 或其他正常无交易结果不得触发第二次账户同步。
...
主交易链路由 PipelineOrchestrator 和 OrchestrationBusinessConnector 推进。订单生命周期同步由独立管线消费已经存在的 OrderSubmissionAttempt，不内嵌在主交易编排尾部。
...
运行巡检问题；
复盘数据集记录；
复盘数据集导出请求、导出包元数据、manifest、下载审计和缺失事实摘要；
AlertEvent；
...
ReviewDataset 是后台生成和导出复盘数据集的能力。

它只读取已落库事实，把四小时周期、编排、行情、策略分析、账户、价格、订单、成交、告警、巡检和审计事实整理成可下载数据集。

ReviewDataset 不请求 Binance，不调用大模型，不影响交易主流程，不由 RuntimeGuard 巡检。后续离线复盘由项目外部的 Codex skill 或人工工具读取导出数据完成，复盘结论不写入生产数据库。
