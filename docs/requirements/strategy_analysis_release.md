...
StrategyAnalysisRelease 的正式候选版本包必须通过 OpsConsole 后台受控入口组装。

组装入口只负责选择已经登记入库、已经具备不可变身份的定义和规则，不负责编辑算法代码、修改算法公式或临时创建新的业务含义。
...
OpsConsole 组装页面必须从已登记的组件库中列出可被正式版本包选择的组件。
...
### 5.5 ReleaseItem 写入规则

每个被选组件必须写入独立 ReleaseItem。
...
后台组装 draft 时必须提供依赖闭包预校验，帮助管理员在冻结前发现缺失组合。
