...
例如 `Performance` 可以作为 Dashboard 图表与 Run 详情页的一部分，也可以独立成页面。
...
```text
首页主收益曲线使用 PerformanceMetrics.cycle_floating_pnl；
订单 realized_pnl 只作为订单级辅助指标；
手续费只作为执行成本辅助指标；
订单净收益只用于订单详情或辅助图表。
```

Dashboard 不得自行计算周期收益。

Dashboard 不得用订单已实现收益代替周期浮动收益。
...
PerformanceMetrics 缺失只表示后台一键补算尚未覆盖该周期或该周期暂不可计算，不等于自动交易主链路异常，也不由 RuntimeGuard 巡检。
...
## 14. PerformanceMetrics 展示

OpsConsole 读取 `OrchestrationRunPerformance` 展示绩效和复盘上下文。
...
```text
周期主图使用 cycle_floating_pnl；
订单 realized_pnl 作为辅助字段；
手续费作为执行成本字段；
calculation_status 非 calculated 时必须显示原因；
收益缺失不得用订单收益或账户余额差额替代。
```

OpsConsole 不自己计算绩效结果。

OpsConsole 必须提供 PerformanceMetrics 后台一键补算入口。

后台一键补算必须调用 PerformanceMetrics service。
...
重复点击不得重复生成已有有效绩效记录；
已存在有效绩效记录的周期必须跳过；
缺少自动边界账户快照的周期只能显示不可计算原因；
PerformanceMetrics 缺失不等于自动交易主链路异常；
...
AIReview 页面用于创建离线复盘请求、查看复盘状态、下载复盘数据包、查看大模型复盘报告、查看问题发现和人工建议。
...
提交 AIReview 任务时必须包含 `review_mode`。

提交 AIReview 任务时只能选择后端允许的 `model_profile_code`。
...
页面只负责选择 `review_mode`、选择数据范围、选择允许的 `model_profile_code`、填写人工问题、提交任务、展示结果和管理建议人工状态。

具体数据组装、prompt 构造、DeepSeek 调用、报告保存、状态流转和失败处理由 AIReview 模块定义。

AIReview 页面至少支持：

```text
创建 AIReviewRequest；
查看 AIReviewRequest 状态；
查看 AIReviewPackage 摘要；
下载 AIReviewPackage；
查看 AIReviewAttempt 摘要；
查看 AIReviewReport；
查看 AIReviewFinding；
查看 AIReviewSuggestion；
...
AIReview 结果只供离线分析。大模型复盘结论不得自动影响实时策略、风控或执行。
...
预览 PerformanceMetrics 缺失周期；
一键补齐 PerformanceMetrics 缺失周期；
创建 AIReview 离线复盘请求。
```
...
review_exporter 可以创建和查看离线复盘请求、下载复盘数据包和报告；
admin 可以管理用户和权限；
查看权限不自动包含操作权限；
复盘权限不自动包含订单补查权限；
运维操作权限不自动包含用户管理权限。
...
## 29. 与 PerformanceMetrics 和 AIReview 的关系

OpsConsole 使用 PerformanceMetrics 结果展示收益。

OpsConsole 不计算周期收益。

OpsConsole 可以创建 AIReview 离线复盘请求，并展示 AIReview 保存的报告、发现和建议。

AIReview 相关规则：

```text
只离线分析；
只选择后端允许的 model_profile_code；
不提交完整 model_profile、任意模型名、base_url、API key 或 provider 参数；
不参与实时交易；
不自动变更策略；
不自动变更风控；
不自动变更执行；
...
调用大模型：前端否；OpsConsole 后端自身不直接调用，由 AIReview 通过 DeepSeekGateway 处理离线复盘调用。
...
4. review_exporter 可以创建和查看复盘请求但不能补查订单。
5. Dashboard 使用 PerformanceMetrics.cycle_floating_pnl 作为主收益。
6. Dashboard 不把 realized_pnl 当成周期收益。
...
21. PerformanceMetrics 缺失扫描预览不写绩效记录。
22. PerformanceMetrics 一键补齐需要权限、理由和审计，且不会重复生成已有有效记录。
23. AIReview 复盘请求只能选择 model_profile_code，不能提交完整 model_profile、任意模型名或 provider 参数。
24. AIReview 数据包和报告下载不包含密钥、token、签名或完整认证 header。
25. 危险操作必须二次确认。
26. 所有人工操作写审计记录。
27. 前端不能直接访问数据库。
...
Dashboard 使用 PerformanceMetrics 主收益口径；
PerformanceMetrics 通过后台一键补齐缺失周期，不按单周期手工 confirm-write 作为主入口；
RuntimeGuardIssue 与原业务对象状态清晰区分；
AlertEvent 区分业务事件、编排事件和巡检事件；
订单提交在任何后台入口都不重试；
ActiveLock 不由页面直接变更；
AIReview 只作为离线复盘入口，不参与实时交易；
AIReview 页面不能提交完整模型配置、任意模型名或 provider 参数；
危险操作具备权限、二次确认和审计；
