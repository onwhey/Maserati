...
| FeatureCode | 业务含义 | 值类型 | 最小窗口 |
|---|---|---|---:|
| risk_bars_since_market_shock_4h_6 | 最近一次“振幅达到 7% 或实体涨跌幅绝对值达到 4%”的 4h K 距离当前多少根；当前即冲击为 0；最近 6 根均无冲击时为 7 | integer | 7 |

计算规则：

```text
只读取当前 MarketSnapshot 冻结的已收盘 4h K；
从当前 K 向前检查 6 根，共检查 7 根；
单根振幅统一使用 (high - low) / open；
单根实体涨跌幅统一使用 (close - open) / open；
找到最近一根振幅 >= 7% 或实体涨跌幅绝对值 >= 4% 的 K 后返回距今根数；
没有找到时返回 observation_window + 1，即 7。
```

该特征只表达“冲击距离当前有多远”，不直接表达观察期、风险状态或交易动作。

### 6.7 高波动无方向基础事实

| FeatureCode | 业务含义 | 值类型 | 最小窗口 |
|---|---|---|---:|
| risk_direction_flip_count_4h_8 | 最近 8 根 4h K 实体方向切换次数；十字 K 不参与方向序列 | integer | 8 |
| risk_movement_efficiency_4h_8 | 最近 8 根 4h 收盘净位移 / 收盘路径总位移 | decimal | 8 |
| risk_cumulative_return_pct_4h_8 | 最近 8 根 4h 收盘累计涨跌幅 | decimal | 9 |
| risk_cumulative_range_pct_4h_8 | 最近 8 根 4h 的 `(high - low) / open` 之和 | decimal | 8 |
...
方向切换多，只说明涨跌交替频繁；
移动效率低，只说明净位移相对路径较小；
累计振幅大，只说明市场走过的总波动路径较长；
这些事实必须由 AtomicSignal 组合后，才能判断“高波动无方向”。
```
...
| 冲击后观察窗口 | 6 根 4h | 极端振幅事件发生后的事实回看范围 |
| 高波动无方向窗口 | 8 根 4h | 统计方向切换、移动效率、累计收益和累计振幅 |
...
最近 8 根涨跌交替时方向切换次数正确；
最近 8 根移动效率和累计振幅可复算；
最近 6 根没有极端振幅时 bars_since 返回 7，不用 0 伪装当前冲击；
