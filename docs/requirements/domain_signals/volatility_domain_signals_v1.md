...
```text
volatility 领域为低波动压缩：低波动证据 3 项成立，高波动证据 0 项，极高波动证据 0 项；其中 4h 短窗口波动低于长窗口波动，说明短周期波动处于压缩状态。该结论只描述波动收缩，不代表即将突破或任何交易处理。
```

高波动示例：

```text
volatility 领域为高波动：高波动证据 3 项成立，低波动证据 0 项，极高波动证据 0 项；其中 4h ATR 分位较高且短窗口波动扩张，说明当前波动明显放大。该结论不代表方向性交易处理。
```

极高波动示例：

```text
volatility 领域为极高波动：极高波动证据 1 项成立，说明当前波动处于异常强烈状态。是否影响信号质量或下游消费条件，应由 risk_state、StrategySignalQuality 或后续模块判断。
```
...
Volatility DomainSignal 也不得把低波动压缩直接解释成“趋势中继整理”，不得把高波动直接解释成“宽幅震荡”，不得把极高波动直接解释成“本轮不形成交易目标”。
