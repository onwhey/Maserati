...
## 15. 后续版本待补：结构受压、结构破坏与结构修复事实

当前 `structure` v1 已经能表达：
...
但在 MarketRegime v3 的验收中暴露出一个重要缺口：

```text
MarketRegime 需要知道“结构是否保持、受压、破坏或修复”，
而当前 structure v1 更多只是输出“价格现在在哪个位置”，
对“这个位置相对于前一段结构意味着什么”暴露不足。
```

后续 `structure` v2 或更高版本应优先补充以下客观结构事实：
...
```text
多头高位结构受压
只表示上涨背景下，高位结构、压力区、动能或短周期结构已经出现压力。
它不表示应该做空，不表示应该减仓，也不表示牛市已经结束。
...
后续如要实现这些状态，应先补齐对应 Feature / AtomicSignal 需求，明确：
...
```text
Structure DomainSignal 只暴露客观结构事实；
不得判断这是牛市回调还是熊市开始；
不得判断这是熊市反弹还是牛市开始；
不得输出策略选择；
不得输出目标仓位；
...
MarketRegime 可以消费这些结构事实来判断市场环境。

StrategyDefinition 可以消费这些结构事实来决定具体策略行为。

但 Structure DomainSignal 自身不得越权到 MarketRegime、StrategyRouting、StrategySignal 或 DecisionSnapshot。
