...
DomainSignal 的正式输出分为两类内容：

```text
核心结论：领域必须给出的主判断，例如 direction、state_code、strength、agreement_ratio 和核心证据摘要；
可选证据块：只在当前版本包纳入对应原子信号时出现的补充证据，例如长周期历史结构位、江恩时间参考、资金费率参考或其他实验性证据。
```

可选证据块不是临时特例，而是领域层的通用扩展机制。

每个可选证据块必须在领域定义或领域实现说明中声明：

```text
block_code：证据块代码；
display_name：人类可读名称；
所属领域；
该证据块依赖哪些 AtomicSignal；
哪些 AtomicSignal 是该证据块内部必需，哪些只是补充；
该证据块是否允许影响领域核心结论；
该证据块是否允许进入 evidence_text_zh；
该证据块无数据、无效或证据不足时如何表达；
该证据块是否属于正式必跑证据。
```
...
```text
如果当前 StrategyAnalysisRelease 没有纳入某个可选证据块依赖的任何原子信号，该证据块不得出现在 DomainSignal payload 或 evidence_text_zh 中；
如果纳入了该证据块依赖的原子信号，但证据不足或计算无效，该证据块可以出现，但必须明确 is_valid=false、reason_code 或等价说明；
如果某个证据块被定义为正式必跑证据，则其必需原子缺失或无效时，该领域不得作为完整正式领域继续向下游输出；
可选证据块不得绕过 AtomicSignal 直接读取 FeatureValue、Kline 或其他底层事实；
可选证据块不得生成交易动作、策略选择、目标仓位或订单语义。
```

可选证据块能否影响核心结论，必须显式声明。

```text
默认不影响 direction、state_code、strength、agreement_ratio；
只有当需求文档明确说明该证据块已经升级为核心证据，才允许进入核心聚合逻辑；
从可选证据升级为核心证据，必须形成新的领域定义版本或新的领域依赖配置版本。
```

因此，类似江恩时间机制、长周期历史结构位、链上指标、资金费率异常等尚需验证有效性的内容，应优先作为可选证据块接入。

```text
验证期间：选中对应原子时出现，取消对应原子时完全不显示；
验证无效：取消对应原子即可从版本包和领域输出中移除；
验证有效：再通过文档、测试和新版本定义决定是否升级为核心证据。
```
