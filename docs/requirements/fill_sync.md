...
保存查询、计算、完整性和锁收尾证据；
为 OpsConsole、PerformanceMetrics 和 AIReview 提供已落库成交事实。
```
...
FillSync 只记录交易所事实，不把 realized pnl 直接写入账户权益、OrchestrationRunPerformance 或 BinancePositionSnapshot。
...
TradeFill 和 OrderFillSummary 是成交事实。OpsConsole、PerformanceMetrics 和 AIReview 可以通过对应后端 service 读取这些已落库事实，不需要 FillSync 额外生成 Tracking 交接对象或事件。
...
涉及 PerformanceMetrics / AIReview：只提供已落库成交事实与汇总，不直接调用；
