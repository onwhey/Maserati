...
OrderPlan 可以读取 `target_position_ratio`，但不得重新解释 StrategySignal 或重新执行 DecisionPolicyCalculator。
...
算法需求文档负责定义目标仓位决策公式、输入、参数、边界、输出语义和验证要求，应放在 requirements 下的对应决策算法目录，具体目录由后续决策算法需求文件统一确定，例如：

```text
docs/requirements/<决策算法模块>/<decision_policy_or_algorithm>.md
```
...
NO_TRADE / NO_TARGET_CHANGE 不触发账户同步，且不生成 PriceSnapshot；
...
涉及 OrderStatusSync / FillSync / PerformanceMetrics / AIReview：不直接调用，只作为后置追溯事实；
