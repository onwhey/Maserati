...
AIReviewReport 主存储；
AIReviewPackage / AIReviewAttempt / AIReviewFinding / AIReviewSuggestion 主存储；
...
涉及真实交易、人工 ActiveLock 收尾、PerformanceMetrics 后台一键补算或真实交易运行开关的 command 必须显式说明：
...
BinanceGateway 涉及账户、价格、订单、成交和交易规则的调用必须携带并校验 active market domain；
DeepSeekGateway 和 Notifications 不以 active market domain 作为通用必填上下文；
...
业务模块直接创建 DeepSeek client；
OpsConsole 直接调用 BinanceGateway；
OpsConsole 直接调用 DeepSeekGateway；
...
## 22. DeepSeekGateway 基础依赖

ProjectFoundation 必须为 DeepSeekGateway 提供配置、日志、测试和安全基础。
...
DeepSeek API key 配置；
base_url 配置；
model profile 预设套餐配置；
业务调用方只选择套餐编号，不传完整模型配置；
超时配置；
限频、冷却和熔断配置；
结构化输出相关配置；
fake gateway 测试替换。
...
```text
DeepSeek API key 不得保存到数据库；
DeepSeek API key 不得由 OpsConsole 管理；
OpsConsole 不直接调用 DeepSeekGateway；
默认测试不得访问真实 DeepSeek；
日志、异常、指标、AlertEvent 和前端不得泄露完整 prompt、完整输出或 API key。
```

## 23. Notifications 基础依赖
...
## 27. dry-run 与 confirm-write 基础规则
