...
实验演进版本允许为了快速验证问题而复用上一版代码，例如通过继承、调用公共评分函数或只覆盖局部判断逻辑。

但一旦某个版本准备作为稳定候选进入大样本回测或正式版本包候选，就不得继续把核心分类逻辑分散在多层继承链里。

稳定候选版本必须做到：

```text
一份版本文件能读清楚完整分类流程；
核心候选评分、优先级、tie-break、降级逻辑在当前版本中明确表达；
可以复用通用工具函数，例如分数截断、排序、领域事实解析和输出格式化；
不得依赖“先读 v1，再读 v2，再读 v3，最后才能理解 v4”的维护方式；
不得为了少写代码而让正式候选算法长期层层继承实验版本。
```

因此：

```text
v1 / v2 / v3 可以视为 MarketRegime 的实验演进链；
后续 v4 如果作为结构证据增强版稳定候选，应独立表达完整分类逻辑；
v4 可以参考 v2 / v3 已验证有效的规则，但不能把核心逻辑继续隐藏在 v2 / v3 的 super 调用里。
```
...
但 MarketRegime 的业务判断主体必须在当前算法版本中可审计、可复盘、可维护。
