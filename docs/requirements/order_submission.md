...
为 accepted / unknown 登记或触发独立订单生命周期同步；
...
Execution 只能使用 PreparedOrderIntent 已冻结的数量、执行准备阶段冻结的价格事实，以及明确绑定的 BinanceSymbolRuleSnapshot 计算本次订单名义。

```text
USDS-M MARKET order_notional
= quantity * selected_live_price

USDS-M LIMIT order_notional
...
```text
使用 Decimal 或等价精确十进制；
MARKET 的 selected_live_price 必须来自已通过的 ExecutionPreparationResult；
LIMIT 的 limit_price 必须来自 PreparedOrderIntent 冻结字段；
...
side 只能是 BUY 或 SELL；
quantity 大于零；
price = PreparedOrderIntent.limit_price；
timeInForce = PreparedOrderIntent.time_in_force；
goodTillDate 或等价到期字段仅在 Gateway 明确支持且 PreparedOrderIntent 已冻结时发送；
positionSide = BOTH，或由 Gateway 按 One-Way Mode 的固定协议省略；
newClientOrderId = PreparedOrderIntent.client_order_id；
reduceOnly = PreparedOrderIntent.reduce_only；
不发送 stopPrice；
...
LIMIT 提交只表示交易所接受了挂单请求，不表示已经成交。Execution 不等待成交，也不因为限价单可能长期挂单而延长提交动作。
...
即使 MARKET 或 LIMIT 响应中出现 `FILLED`，本模块也只记录提交响应，不直接创建 TradeFill，不因响应看起来已成交而释放 ActiveLock。
...
登记或触发独立订单生命周期同步管线，由 OrderStatusSync 使用原 client_order_id 查询。
```
...
Execution 本身不得因为 MARKET 单“通常立即成交”或 LIMIT 单“已经被交易所接受”而释放锁。

## 29. 订单生命周期同步交接

accepted 或 unknown 后，主交易编排不得在同一个 run 内继续执行 OrderStatusSync / FillSync。

Execution 只负责产生交接事实或事件；独立订单生命周期同步管线必须消费同一 `OrderSubmissionAttempt`，并交给 OrderStatusSync。
...
Execution 不在本模块内实现轮询，也不得等待订单终态或成交事实。
...
12a. LIMIT payload 必须包含冻结的 price 和 timeInForce，不得包含 stopPrice。
...
41. accepted / unknown 登记或触发独立订单生命周期同步管线。
...
52. USDS-M MARKET 使用冻结数量乘 selected_live_price 记录订单名义；USDS-M LIMIT 使用冻结数量乘 limit_price 记录订单名义。
...
accepted / unknown 进入独立订单生命周期同步管线；
