...
计算目标名义、目标数量或目标合约张数；
计算当前仓位与目标仓位差额；
读取 DecisionSnapshot 冻结的交易价格条件；
按交易规则规范化订单数量；
判断是否无需调仓；
判断本轮适合市价执行、限价等待、等待更好价格或不生成订单；
...
OrderPlan 可以读取 DecisionSnapshot 冻结的 `frozen_trade_price_condition`。

该条件只用于判断候选订单规划方式，不是交易所订单参数。

OrderPlan 不得重新读取 StrategySignal、StrategyRouting、MarketRegime、DomainSignal 或 AtomicSignal 来解释价格条件。OrderPlan 只能消费 DecisionSnapshot 已冻结的价格条件快照，以及本轮明确传入的 PriceSnapshot。

`frozen_trade_price_condition` 若存在，必须使用 StrategySignal 标准合同中的结构：
...
```text
condition_type = 当前策略给出的价格条件类型，例如靠近支撑、靠近压力、突破后不追价、无价格约束等；
reference_price_zone = 策略引用的关键价格区域，用于审计和解释；
acceptable_price_zone = 策略认为可以执行的价格区域，用于 OrderPlan 判断当前价格是否可执行，或是否能形成 LIMIT 价格；
support_or_resistance_refs = 价格区域的证据引用，必须能追溯到已冻结的结构领域事实或策略证据；
allow_chasing = 是否允许在当前价格未处于理想区域时继续按 MARKET 规划；
reason_code / reason_summary_zh = 策略给出该价格条件的原因，OrderPlan 必须原样保留到计划证据中。
```

如果 `frozen_trade_price_condition` 存在但缺少必要字段、字段无法解析、价格区域无法校验或证据引用无法追溯，OrderPlan 必须 fail-closed：

```text
OrderPlan.status = blocked；
reason_code = invalid_frozen_trade_price_condition；
不生成 CandidateOrderIntent；
不取得 ActiveLock；
写 AlertEvent。
```
...
价格条件允许追价，且本轮 PriceSnapshot 位于可接受价格区间内
→ 可以按 MARKET 候选订单继续规划；

价格条件允许追价，但本轮 PriceSnapshot 不在可接受价格区间内
→ 不得因为 allow_chasing=true 就无条件追价，必须按 condition_type 和 acceptable_price_zone 判断是否仍可生成 LIMIT 或 no_order_required；

价格条件不允许追价，但存在明确可接受价格区间
→ 可以生成 LIMIT 候选订单；

价格条件不允许追价，且缺少可执行的限价边界
→ no_order_required 或 blocked，具体由 reason_code 说明；
...
```text
limit_price 必须来自 frozen_trade_price_condition.acceptable_price_zone、support_or_resistance_refs 和本轮 PriceSnapshot 可验证输入，不能请求 Binance；
limit_price 不得来自策略文案、人工自由文本或运行时临时猜测；
reference_price_zone 只能作为审计解释，不能单独替代可执行的 acceptable_price_zone；
allow_chasing = false 时，若当前 PriceSnapshot 不在 acceptable_price_zone 内，OrderPlan 不得生成 MARKET 候选订单；
allow_chasing = true 只表示策略允许追价，不表示 OrderPlan 可以绕过交易规则、最小调仓、ActiveLock 或风控前置合同；
limit_valid_until_utc 默认不得晚于本 4 小时周期结束前 10 分钟；
0 点周期生成的限价单，默认有效到 03:50 UTC；
4 点周期生成的限价单，默认有效到 07:50 UTC；
限价单不得跨入下一轮主编排；
OrderPlan 只生成候选限价意图，不提交、不撤单、不管理挂单生命周期。
```

如果策略价格条件只表达“等待更好价格”，且当前无法得到可报单的 limit_price，OrderPlan 应返回 `no_order_required`，而不是生成一个随意价格。

`no_order_required` 表示本轮目标仓位方向存在，但当前价格条件无法形成可执行候选订单；这不是策略失败，也不是风控拒绝。

最小调仓金额、交易所最小数量和交易所最小名义仍由 OrderPlan 的数量规范化与最小调仓判断负责，不得前移到 DecisionSnapshot 或 StrategySignal。
...
One-Way Mode 的净额反手可以生成一笔候选订单，但必须拆分内部风险组件。候选订单类型由 OrderPlan 根据目标仓位差额、PriceSnapshot 和 DecisionSnapshot 冻结的价格条件确定。
...
## 28. OrderStatusSync 等待边界

Execution accepted 后，独立订单生命周期同步管线将交给 OrderStatusSync 按其需求进行有限状态同步：
...
49. 无价格条件且需要调仓时可以生成 MARKET 候选订单。
50. 价格条件允许追价且 PriceSnapshot 位于可接受区间时可以生成 MARKET 候选订单。
51. 价格条件不允许追价且存在明确可接受价格区间时可以生成 LIMIT 候选订单。
52. LIMIT 候选订单必须冻结 limit_price、limit_valid_until_utc、time_in_force_policy 和价格条件证据 hash。
53. LIMIT 候选订单的有效期不得跨入下一轮主编排。
54. 价格条件与目标方向矛盾时 blocked，不临时改方向。
```
