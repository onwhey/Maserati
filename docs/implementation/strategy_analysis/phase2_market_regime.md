...
context_structure_regime / v1 calculator；
seed_market_regime_definitions 默认定义登记命令；
...
requirement = docs/requirements/market_regime/context_structure_regime_v1.md
implementation = docs/implementation/market_regime/context_structure_regime__v1.md
```

正式版本包仍必须显式选择一个合法、可解析、已批准的 MarketRegimeDefinition。

如果版本包没有选择 MarketRegimeDefinition，MarketRegime 正式入口仍然 fail-closed，也就是返回 blocked，不生成可供 StrategyRouting 消费的 MarketRegimeSnapshot。
...
正式结果与 dry-run 严格隔离。dry-run 即使遇到相同业务幂等键或相同输入身份，也会执行相同 calculator 校验并返回 `persisted = false`，不会返回正式 Snapshot，也不会放行 StrategyRouting。

不支持 dry-run 的 calculator 会在计算前阻断。

业务幂等键命中历史 Snapshot 时，Service 会同时核对 DomainSignalSet、StrategyAnalysisRelease 和 Definition 身份；同一个幂等键指向不同输入时阻断，不返回其他周期的结果。并发唯一键冲突后执行相同身份校验。
...
明确数据库数据错误归为 failed；
context_structure_regime / v1 的高风险、突破、震荡和完整评分输出。
```
