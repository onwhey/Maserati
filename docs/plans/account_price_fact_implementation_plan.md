...
为 ReviewDataset 提供连续 UTC 4 小时账户边界事实；
...
本文档不实现 OrderPlan、RiskCheck、ExecutionPreparation、Execution、订单状态、成交同步、ReviewDataset 导出、后台页面或真实下单。
...
无论是否交易，都作为 ReviewDataset 的 UTC 4 小时周期边界账户事实。
```
...
不得使用 ops_display 兜底；
不得使用 Redis 缓存账户事实兜底；
不得为了 ReviewDataset 补造账户边界；
编排 adapter 必须返回 STOP / FAIL 或等价不可继续结果。
```

ReviewDataset 后续只能看到该周期缺少边界快照，并记录缺失事实原因。
...
其中 ReviewDataset 只读取自动边界 `trade_preparation` 快照，不请求 Binance，不补刷新账户。
...
不得被 OrderPlan、RiskCheck 或 ReviewDataset 自动作为交易账户边界消费；
不得改变真实交易允许状态。
```
...
ops_display 不能被交易 selector 或 ReviewDataset 作为交易账户边界消费；
失败只影响本次展示刷新。
```
...
ReviewDataset 依赖本阶段的 `trade_preparation BinanceSyncRun` 作为 UTC 4 小时周期边界账户事实。
...
ops_display 快照不作为 ReviewDataset 的交易账户边界；
ReviewDataset 不要求 BinanceAccountSync 为它额外刷新账户。
```

本阶段不实现 ReviewDataset 导出。
...
后续 ReviewDataset 能通过编排关联找到周期边界 BinanceSyncRun。
```
...
### 18.4 ReviewDataset 边界测试

本阶段不构建 ReviewDataset，但必须测试账户边界事实可供后续识别：
...
后续无交易、NO_TRADE、NO_TARGET_CHANGE 或 no_strategy 时，该快照仍可作为 ReviewDataset 边界事实；
同步失败不伪造边界快照，不回退历史批次；
ops_display 只用于后台展示，不进入交易链路，也不作为 ReviewDataset 交易账户边界；
...
ops_display 被交易 selector 或 ReviewDataset 作为交易账户边界消费；
...
在进入下一阶段前，不应开始订单提交、订单状态查询、成交同步、通知投递或 ReviewDataset 能力。
