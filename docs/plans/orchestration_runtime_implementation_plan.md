...
```text
PipelineOrchestrator
→ Execution / OrderStatusSync / FillSync 交易关键工作项
→ 交易关键 worker
→ 原 OrchestrationRun 的 WAIT 恢复入口。
```
...
本阶段不把 AIReview 和 PerformanceMetrics 接入交易关键任务组；其离线任务路由在下一阶段计划中实现。
...
进入 Execution、OrderStatusSync 或 FillSync 时：

```text
driver 保存明确 StepRun、业务对象引用和恢复信息；
run 进入 waiting；
投递交易关键工作项；
driver task 结束；
...
→ 明确终态，恢复 run 并进入 FillSync
→ 30 秒仍未解决，run 结束为 unknown，ActiveLock 保持。
```
...
resume_token 只能消费一次；
恢复不重做已完成步骤；
Execution、OrderStatusSync、FillSync 使用交易关键任务组；
同一 poll_sequence 只查询一次；
两秒节奏和 30 秒窗口正确；
...
RuntimeGuard 巡检 AIReview 或 PerformanceMetrics；
