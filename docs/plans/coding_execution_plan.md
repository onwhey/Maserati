...
阶段 7：已完成 OpsConsole 后端查询、受控人工操作入口、PerformanceMetrics 后台补算、DeepSeekGateway、AIReview 后端与前端基础入口。
```
...
RuntimeGuard 不补跑、不修复、不释放锁、不调用 Binance、不调用 DeepSeek，不巡检 AIReview / PerformanceMetrics / 后台人工离线任务。
```
...
```text
PerformanceMetrics 后端对象与补算 service；
PerformanceMetrics 后台预览与一键补算 API；
只读取已落库事实，只使用 trade_preparation 账户边界快照；
不请求 Binance，不进入自动交易主链路，不生成交易信号，不影响交易执行。
```
...
本批次不实现 AIReview 自动调度，不实现真实 DeepSeek 调用。
```
...
```text
OpsConsole Performance 前端页面；
PerformanceMetrics 已落库记录列表；
周期浮动收益图表；
缺失周期预览；
后台一键补算表单；
...
```text
OpsConsole AIReview 前端创建请求表单；
AIReview 详情页人工操作入口；
构建 AIReviewPackage；
人工确认后执行离线 AIReview；
更新 AIReviewSuggestion 人工状态；
前端只提交复盘范围、复盘模式、受控模型套餐编号和人工说明；
前端不上传业务事实，不直接调用 DeepSeek，不执行 AI 建议。
```
