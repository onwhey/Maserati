...
实现每四小时自动账户边界快照；
实现 PriceSnapshot；
形成 OrderPlan / RiskCheck / ExecutionPreparation / ReviewDataset 可读取的账户和价格事实。
```
...
ReviewDataset 后续只使用自动边界账户快照作为交易账户边界；
...
本阶段不得实现通用撤单、改单、自动修复、模拟交易运行模式。

已由 `order_cycle_closeout.md` 定义的限价单周期收尾撤单只允许按该需求接入，不得扩展为通用人工撤单、改单或追单能力。
...
本阶段没有实现自动恢复、自动补跑、自动释放 ActiveLock、真实 Hermes 外发、ReviewDataset 巡检或后台人工补算。

阶段 6 完成后，阶段 7 的执行文件为：
...
ReviewDataset 只读取已落库事实，不请求 Binance；
ReviewDataset 不调用外部大模型；
ReviewDataset 由后台创建和下载，不是自动主链路步骤；
离线复盘由项目外部 Codex skill 或人工工具消费导出文件，结果不写入生产数据库。
```
...
RuntimeGuard 不巡检 ReviewDataset 或后台人工离线任务。
```
...
阶段 2：策略分析框架，已完成从 FeatureLayer 到 DecisionSnapshot 的框架落地。
...
阶段 6：已完成 PipelineOrchestrator、Notifications 与 RuntimeGuard 第一版可运行骨架。
阶段 7：已完成 OpsConsole 后端查询、受控人工操作入口和 ReviewDataset 数据集导出入口。
```
...
OrchestrationRun、OrchestrationStepRun、OrchestrationBusinessObjectLink；
Registry、StepAdapter、OrchestrationBusinessConnector；
Celery task 入口；
编排层只理解 adapter 统一结果，不直接解释业务状态，不直接访问 Binance / DeepSeek，不直接释放 ActiveLock。
```
...
```text
Notifications；
AlertEvent 到 NotificationDeliveryAttempt / NotificationSuppression 的投递决策闭环；
通知失败不回滚业务事实，通知成功不触发业务动作；
本阶段未实现真实 Hermes 外发。
```
...
RuntimeGuard；
独立只读巡检 run / issue 记录；
默认巡检自动编排主链路、订单链路卡住状态、ActiveLock 风险状态和通知投递状态；
RuntimeGuard 不补跑、不修复、不释放锁、不调用 Binance、不调用外部大模型，不巡检 ReviewDataset / 后台人工离线任务。
```
...
```text
OpsConsole 后端基础查询；
Dashboard、Runs、Orders、AccountOverview、RuntimeGuard、Alerts、RealTrading、AuditLog 查询接口；
OpsConsole selector 只读聚合，不直接访问外部服务，不承载业务逻辑。
```
...
```text
OpsConsole 前端基础骨架；
Next.js + TypeScript 页面结构；
Dashboard、Runs、Orders、AccountOverview、RuntimeGuard、Alerts、RealTrading、AuditLog 基础展示页；
前端只调用 Django OpsConsole API，不直连数据库、Gateway、DeepSeek 或 Hermes。
```

阶段 7 已完成第三批次：

```text
ReviewDataset 后端对象与构建 service；
ReviewDataset 后台预览、创建、状态查询和下载 API；
只读取已落库事实，只使用 trade_preparation 账户边界快照作为交易账户边界；
不请求 Binance，不调用外部大模型，不进入自动交易主链路，不生成交易信号，不影响交易执行。
```

阶段 7 已完成第四批次：

```text
OpsConsole ReviewDataset 后端 API 接入；
ReviewDatasetRecord / ReviewDatasetExport 列表与详情查询；
通过 OpsConsole 创建 ReviewDataset、查询状态和下载导出文件；
OpsConsole 不直接访问 MySQL，不调用 BinanceGateway，不调用外部大模型。
```
...
```text
OpsConsole ReviewDataset 前端页面；
ReviewDataset 已落库记录列表；
缺失事实摘要；
数据集创建表单；
下载入口；
补算入口要求 confirm_write 和 reason；
前端不自行计算周期收益，不请求 Binance，不影响主交易编排。
```

阶段 7 不再实现系统内大模型复盘页面；离线复盘由项目外部 Codex skill 或人工工具读取 ReviewDataset 导出文件完成。

当前剩余验收事项：

```text
本机尚未安装 Node.js / npm，OpsConsole 前端 typecheck / build 暂未执行；
安装 Node.js LTS 后，应在 frontend/ops-console 执行 npm install，再执行 npm run typecheck 和 npm run build；
后端已通过 manage.py check、makemigrations --check --dry-run，以及阶段 7 相关 pytest。
```

当前建议下一步：

```text
如果继续编码：先做阶段 7 总收尾复核，确认没有遗漏的后台页面、接口和测试；
如果准备提交：先提交当前已验证的后端与前端源码变更，并在提交说明中标注前端 Node/npm 验证待本机环境安装后补跑；
如果要完整验收前端：先安装 Node.js LTS，再跑 OpsConsole typecheck / build。
```
