"""AtomicSignal 模块：人工调用正式 service；不直接写业务状态、不访问外部服务、不执行交易。"""
...
class Command(BaseCommand):
    help = "根据明确 FeatureSet 和冻结版本包构建 AtomicSignalSet"

    def add_arguments(self, parser):
        parser.add_argument("--feature-set-id", type=int, required=True)
    ...
    def handle(self, *args, **options):
        ...
        items = tuple(
            release.items.filter(component_type=ReleaseItemComponentType.ATOMIC_SIGNAL_DEFINITION).order_by(
                "sort_order", "component_code", "id"
            )
        )
        result = build_atomic_signals(
